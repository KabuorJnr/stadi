...
<div class="flex items-center gap-3">
    ...
    <h1 class="text-2xl font-extrabold text-white">Edit Event</h1>
</div>
...
<form method="POST" action="{{ route('admin.events.update', $event) }}" class="mt-6 max-w-2xl">
    ...
    <div class="mt-6 rounded-xl bg-dark-100 p-4">
        <label class="flex items-center gap-3">
            ...
            <input type="checkbox" name="ticket_sales_open" value="1" {{ old('ticket_sales_open', $event->ticket_sales_open) ? 'checked' : '' }}
                   class="h-4 w-4 rounded border-white/20 bg-dark text-accent focus:ring-accent">
            <span class="text-sm font-medium text-white">Ticket sales open</span>
        </label>
    </div>

    <div class="mt-8 flex items-center gap-4">
        <button type="submit" class="rounded-xl bg-accent px-6 py-2.5 text-sm font-bold text-white transition hover:bg-accent-light">Update Event</button>
